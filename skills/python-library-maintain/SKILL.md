...
4. Run the full quality stack before committing: `uv sync && uv run pytest && uv run ruff check src/ tests/ && uv run pyright src/linkedin_publish/`.
